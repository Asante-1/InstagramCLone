...
function Actions({docId, totalLikes, likedPhoto, handleFocus }) {
  ...
  const [likes, setLkikes] = useState(totalLikes)
  ...
  const handleToggleLiked = async() => {
    ...
    await firebaseapp
        ...
        .doc(docId)
        .update({   
          likes : toggleLiked ? FieldValue.arrayRemove(uid) : FieldValue.arrayUnion(uid)
        });  
        
    setLkikes((likes) => (toggleLiked ? likes -1 : likes + 1))
